...
int main (int argc, char * argv[])
{
    ...
    cin >> temp;
    cout << "# Por búsqueda secuencial ";
    start = clock();
    if((pos=b_secuencial(v, TAM, temp))!=-1)
           cout << temp << " se encuenra en la posición: " << pos << endl;
    else
...
int b_secindexada(int v[], int tam, int buscar){
 ...
 if(intervalo==-1){
  cout << endl << "No se encontró un intervalo adecuado... abortando..." << endl;
  return -1;
 }
 ...
 for(i=0; i<tam_ind; i++)
 {
  cout << "La posición es: " << indice[i].pos << endl;
